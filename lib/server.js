...
server.post('/payload', function(req, res) {
  ...
  Handler.handlePullRequest(event)
    .then(({login, contributions}) => {
      ...
      spawnCommand('git', ['clone', event.repository.ssh_url], {
        cwd: './tmp',
      })
        .then(spawnCommand('git', ['checkout', '-b', 'update-contributors'], {
          cwd: `./tmp/${event.repository.name}`,
        }))
        .then(spawnCommand('../../node_modules/.bin/all-contributors', [
          'add', `${login}`, 'infra,plugin', '--commit',
        ], {
          cwd: `./tmp/${event.repository.name}`,
        }))
        .then(spawnCommand('git', ['push', 'origin', '-u', 'update-contributors'], {
          cwd: `./tmp/${event.repository.name}`,
        }))
        // .then(() => {
